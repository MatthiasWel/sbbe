...
def test_bayes_factor_favors_H0_when_data_matches():
    ...
    assert bf < 1, f"BF (currently {bf}) should favor H0 when data matches expected_probs"
...
def test_bayes_factor_favors_H1_when_data_differs():
    ...
    expected_probs = np.array([0.5, 0.25, 0.25])
    bf, _ = bayes_factor_dirichlet_multinomial(counts, expected_probs)
    assert bf > 1, f"BF (currently {bf}) should favor H1 when data differs from expected_probs"
...
def test_scalar_and_vector_alpha_consistency():
    ...
    bf1, _ = bayes_factor_dirichlet_multinomial(counts, expected_probs, alpha=1)
    bf2, _ = bayes_factor_dirichlet_multinomial(counts, expected_probs, alpha=[1, 1, 1])
    assert np.isclose(bf1, bf2), f"Expected same BF (currently {bf1} and {bf2}) for scalar and vector alpha, got {bf1} vs {bf2}"
